...
// Processes are sorted by burst duration (Preemption)
...
func SJFSchedule(w io.Writer, title string, processes []Process) {
	...
	// Processes are sorted by shortest burst t
	for i := 0; i < len(processes); i++ {
		for j := i + 1; j < len(processes); j++ {
...
// Round Robin Schedule - pre-emptive algorithm
func RRSchedule(w io.Writer, title string, processes []Process) {
	...
	q := make([]Process, 0)
	i := 0
	// Process are sorted by arrival time
	for i < len(processes) || len(q) > 0 {
		if len(q) == 0 {
			if i < len(processes) && processes[i].ArrivalTime <= serviceTime {
				q = append(q, processes[i])
				i++
			}
			serviceTime++
			continue
		...
		if process.BurstDuration > TQ {
			process.BurstDuration -= TQ
			q = append(q, process)
			serviceTime += TQ
		} else {
			...
			totalTurnaround += float64(turnaround)
			completion := process.BurstDuration + process.ArrivalTime + waitingTime
			lastCompletion = float64(completion)
